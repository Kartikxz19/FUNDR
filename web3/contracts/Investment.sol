    ...
    contract Investment {
        ...
        function donateToCampaign(uint256 _id) public payable {
            ...
            require(
                amount <= amountNeeded,
                "Amount greater than what is required to achieve Target"
            );
            //campaign should be before deadline to be able to donate
            require(
                c.deadline >= block.timestamp,
                "Cannot donate, deadline finished"
            );
        ...
        function ownerCampaigns() public view returns (Campaign[] memory) {
            ...
            for (uint i = 0; i < numberOfCampaigns; i++) {
                ...
                if (item.owner == msg.sender) {
                    Campaign[] memory tempCampaigns = new Campaign[](
        ...
        function redeemInvestor(uint256 _id) public payable {
            ...
            for (uint256 i = 0; i < n; i++) {
                if (c.donators[i] == msg.sender) {
                    ...
                    require(
                        c.donations[i] > 0,
                        "You have already redeemed your money"
                    );
